...
public class ImageCodeGenerator implements ValidateCodeGenerator {
	...
	public ImageCode generate(ServletWebRequest request) {
		int width = ServletRequestUtils.getIntParameter(request.getRequest(), "width",
				CSecurityProperties.getCode().getImage().getWidth());
		int height = ServletRequestUtils.getIntParameter(request.getRequest(), "height",
				CSecurityProperties.getCode().getImage().getHeight());
		...
		for (int i = 0; i < CSecurityProperties.getCode().getImage().getLength(); i++) {
		...
		return new ImageCode(image, sRand.toString(), CSecurityProperties.getCode().getImage().getExpireIn());
	}
